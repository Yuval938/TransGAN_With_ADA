...
parser.add_argument('--aug_p', type=float, default=0, help='Initial probability of applying each augmentation')
parser.add_argument('--target_realism_score', type=float, default=0.6,
                    help='Target realism score for adaptive augmentation')
parser.add_argument('--kp', type=float, default=0.05, help='Proportional gain for updating augmentation probability')
parser.add_argument('--ada', type=bool, default=True,
                    help="True value will use ADA,False value will use incremental p value based on epochs.")
...
def train(noise, generator, discriminator, optim_gen, optim_dis,
          ...
          gener_batch_size=args.gener_batch_size, device="cuda:0", p=1):
    gen_step = 0
    real_scores = []
    fake_scores = []
    total_loss_dis = 0.0
    total_gener_loss = 0.0
    ...
    for index, (img, _) in enumerate(train_loader):
        ...
        real_imgs = DiffAugment(real_imgs, policy=args.diff_aug, p=p)  # Apply ADA to real images
        noise = torch.cuda.FloatTensor(np.random.normal(0, 1, (img.shape[0], latent_dim)))

        optim_dis.zero_grad()
        real_valid = discriminator(real_imgs)
        fake_imgs = generator(noise).detach()
        fake_imgs = DiffAugment(fake_imgs, policy=args.diff_aug, p=p)  # Apply ADA to fake images
        ...
        loss_dis.backward()
        optim_dis.step()
        real_scores.append(real_valid.mean().item())
        fake_scores.append(fake_valid.mean().item())
        ...
        if global_steps % n_critic == 0:
            ...
            generated_imgs = DiffAugment(generated_imgs, policy=args.diff_aug, p=p)  # Apply ADA to generated images
    ...
    avg_gener_loss = total_gener_loss / (len(train_loader) // n_critic)

    writer.add_scalar("loss_dis", avg_loss_dis, epoch)
    writer.add_scalar("gener_loss", avg_gener_loss, epoch)

    avg_real_score = np.mean(real_scores)  # E[D-train]
    avg_fake_score = np.mean(fake_scores)  # E[D-generated]
    realism_score = avg_real_score - avg_fake_score  # E[D-train] - E[D-generated]
    return realism_score, avg_real_score, avg_fake_score


def evaluate_validation_set(discriminator, validation_loader, diff_aug_policy, p):
    validation_scores = []
    for img, _ in validation_loader:
        real_imgs = img.type(torch.cuda.FloatTensor)
        real_imgs = DiffAugment(real_imgs, policy=diff_aug_policy, p=p)  # Apply ADA to validation images
        real_valid = discriminator(real_imgs)
        validation_scores.append(real_valid.mean().item())
...
best = 1e4
p = args.aug_p
kp = args.kp  # Proportional gain
# Load validation dataset
transform = transforms.Compose(
    [transforms.Resize(size=(32, 32)), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
validation_set = torchvision.datasets.CIFAR10(root='./data', train=False, download=True, transform=transform)
validation_loader = torch.utils.data.DataLoader(dataset=validation_set, batch_size=30, shuffle=False)
for epoch in range(args.epoch):

    lr_schedulers = (gen_scheduler, dis_scheduler) if args.lr_decay else None

    realism_score, E_D_train, E_D_generated = train(noise, generator, discriminator, optim_gen, optim_dis,
                                                    epoch, writer, lr_schedulers, img_size=32,
                                                    latent_dim=args.latent_dim,
                                                    n_critic=args.n_critic,
                                                    gener_batch_size=args.gener_batch_size, p=p)

    if args.ada:
        # Evaluate discriminator on validation set
        E_D_validation = evaluate_validation_set(discriminator, validation_loader, args.diff_aug, p)
        # Calculate r_v
        r_v = (E_D_train - E_D_validation) / (E_D_train - E_D_generated)
        p = p + kp * (r_v - args.target_realism_score)
    else:
        if ((epoch % 40 == 0) and (epoch != 0)):
            # p = p + kp * (r_v - args.target_realism_score)
            p = p + 0.2
    p = min(max(p, 0.0), 1.0)  # Ensure `p` stays within [0, 1]
    print(f"p value is now {p}")
